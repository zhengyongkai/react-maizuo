...
export default function CustomPage() {
  ...
  const user = useSelector<userState, userSimpleInf>((state) => state.user.userData);
  ...
  const message = useMemo(() => {
    return messageList.map((item, index) => {
      return <ChatItem key={index} data={item} onChange={(id, e) => send(id, e)}></ChatItem>;
    });
  }, [messageList]);
  ...
  return (
    <>
      ...
      <div className={cssCb(['inner-scroll', Styles['custom-body']])} ref={bodyRef}>
        {message}
      </div>
