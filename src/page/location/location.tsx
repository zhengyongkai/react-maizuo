...
function LocationPage(props: locationPageInf) {
  ...
  const locationList = useSelector((state: cityStateInf) => state.location.locationList);
  const [city, setCity] = useState(cityState);
  const [hotList, setHotList] = useState<Array<formatInf>>([]);
  const [list, setList] = useState<Array<{ title: string; items: Array<formatInf> }>>([]);
      ...
      <div className={Styles['city-recommend']}>
        ...
        <div className={Styles['city-tabs']}>
          {hotList.map((item, index) => {
            return (
              <div className={Styles['city-tab']} key={index} onClick={() => onCityClick(item)}>
                {item.name}
              </div>
            );
          })}
